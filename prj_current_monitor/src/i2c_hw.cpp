...
#define I2C_WAIT_FLAG_SET(flag) do { \
...
#define I2C_WAIT_FLAG_RESET(flag) do { \
    uint32_t to = I2C_TIMEOUT; \
    while (i2c_flag_get(I2C0, (flag))) { \
        if (--to == 0) return false; \
    } \
...
bool i2c_hw_write_reg(uint8_t dev_addr, uint8_t reg_addr, uint16_t value) {
    ...
    I2C_WAIT_FLAG_RESET(I2C_FLAG_I2CBSY);

    /* send a start condition to I2C bus */
    i2c_start_on_bus(I2C0);
    /* wait until SBSEND bit is set */
    I2C_WAIT_FLAG_SET(I2C_FLAG_SBSEND);

    /* send slave address to I2C bus */
    i2c_master_addressing(I2C0, (uint8_t)(dev_addr << 1), I2C_TRANSMITTER);
    /* wait until ADDSEND bit is set */
    I2C_WAIT_FLAG_SET(I2C_FLAG_ADDSEND);
    /* clear ADDSEND bit */
    i2c_flag_clear(I2C0, I2C_FLAG_ADDSEND);
    /* wait until TBE bit is set */
    I2C_WAIT_FLAG_SET(I2C_FLAG_TBE);
...
bool i2c_hw_read_reg(uint8_t dev_addr, uint8_t reg_addr, uint16_t *value) {
    ...
    I2C_WAIT_FLAG_SET(I2C_FLAG_RBNE);
    /* read data MSB */
