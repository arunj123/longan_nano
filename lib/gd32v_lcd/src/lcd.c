...
#define dma_wait_send()     do { while (DMA_CHCNT(DMA0, DMA_CH2)); } while(0)
...
{
    return (DMA_CHCNT(DMA0, DMA_CH2) != 0) || (SPI_STAT(SPI0) & SPI_STAT_TRANS);
}
...
void lcd_wait(void)
{
    ...
    // Always wait for pending transmit DMA (DMA_CH2) and SPI idle
    while (DMA_CHCNT(DMA0, DMA_CH2) != 0);
    spi_wait_idle();
