...
#define ENCODER_KEY_EXTI    EXTI_0
#define ENCODER_KEY_IRQn    EXTI0_IRQn
...
// NEW, more robust ISR for rotation.
// It fires on S1's falling edge and checks S2's state to determine direction.
void encoder::rotation_isr() {
    auto now = get_timer_value();
    if ((now - last_interrupt_time) < 5) { // Simple debounce for rotation
        exti_interrupt_flag_clear(ENCODER_S1_EXTI);
        return;
    }

    if (gpio_input_bit_get(ENCODER_S2_PORT, ENCODER_S2_PIN) == RESET) {
        rotation_count--; // Was ++ (This reports counter-clockwise / left turn)
    } else {
        rotation_count++; // Was -- (This reports clockwise / right turn)
    }
...
// ISR handler for the key press (with robust debounce)
